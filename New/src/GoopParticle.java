...
public class GoopParticle extends Particle
{
    ...
    public void move()
    {
        ...
        if (spaces.size() != 0)
        {
            ...
            Particle chosen = spaces.get((new Random().nextInt(spaces.size())));

            //switch this particle with that particle
            Grid.getGrid().swapParticles(chosen, this);

            //you dont have to mark this particle as moved - Grid.getGrid().swapparticles does that automatically. all the other move methods were written before this functionality existed/before I realized it was redundant
        }
    ...
    {
        int goopCount = 0;
        for (Particle adj : p.wideNeighbors((Grid.getGrid().particlesPerEdge + 1) / 2))
        {
    ...
    private void doStuff()
    {
        ...
        for (Particle adj : neighbors)
        {
            ...
            if (!adj.type.equals("Air") && !adj.type.equals("Goop"))
            {
                ...
                Grid.getGrid().replaceParticle(this, adj.type);
