...
{
    public static boolean alphaSleep = false;
    public static boolean renderParticleWideNeighbors = false;
    ...
    //this particle's index in the Grid.getGrid().particleList
    ...
    public void updateCoordsFromIndices()
    {
        ...
        x = Grid.getGrid().cornerX + indices.x * particleWidth;
        y = Grid.getGrid().cornerY + indices.y * particleWidth;
    }

    //updates this particle's indices in the particleGrid.getGrid() (also updates screen coords)
    ...
    //   prevType = this.type;
    //   this.type = type;
    //   this.colour = Grid.getGrid().colorMap.get(type).copy();
    ...
    public void render(Main drawer)
    {
        ...
        if (!type.equals("Air"))
        {
            ...
            drawer.rect(x, y, particleWidth, particleWidth);

            if (awake && alphaSleep)
            {
                drawer.line(x, y, x + particleWidth, y + particleWidth);
            }
            drawer.strokeWeight(1);
        }
        ...
        if (renderParticleWideNeighbors)
        {
            ...
            drawer.fill(new Color(255, 255, 255));
            drawer.textSize(10);
            drawer.text(numWideNeighbors, x + particleWidth / 2, y + particleWidth * .75f);
        }
    }
    ...
    public void move()
    {
        System.out.println("a " + type + " particle had no move method?");
    }
    ...
    {
        System.out.println("a " + type + " particle had no interact method?");
    }

    //gets the particle from the Grid.getGrid() that is to the left of this one
    public Particle adjacentLeft()
    {
        ...
        if (xIndex != 0)
        {
            return Grid.getGrid().particleGrid[xIndex - 1][yIndex];
        }
        return null;
    }

    //gets the particle from the Grid.getGrid() that is to the right of this one
    public Particle adjacentRight()
    {
        int xIndex = indices.x;
        int yIndex = indices.y;

        if (xIndex != Grid.getGrid().particleGrid.length - 1)
        {
            return Grid.getGrid().particleGrid[xIndex + 1][yIndex];
        }
        return null;
    ...
    //gets the particle from the Grid.getGrid() that is above this one
    public Particle adjacentUp()
    {
        ...
        if (yIndex != 0)
        {
            return Grid.getGrid().particleGrid[xIndex][yIndex - 1];
        }
        return null;
    }

    //gets the particle from the Grid.getGrid() that is below this one
    public Particle adjacentDown()
    {
        int xIndex = indices.x;
        int yIndex = indices.y;

        if (yIndex != Grid.getGrid().particleGrid[0].length - 1)
        {
            return Grid.getGrid().particleGrid[xIndex][yIndex + 1];
        }
        return null;
